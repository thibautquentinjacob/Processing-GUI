...
/* ===========================
    Slider :: Slider
 =========================== */
    public Slider( int x, int y, int min, int max, int size ) {
        this.x = x;
        this.y = y;
        this.range = max - min;
    ...
    public Slider( int x, int y, int min, int max, int size, ArrayList<Integer> dragPositions ) {
        this.x = x;
        this.y = y;
        this.range = max - min;
        this.resolution = this.range / size;
    ...
    public void draw() {
        ...
        for ( PVector overlay : ranges ) {
            rect( overlay.x * width / this.range, this.y + this.height + 2, overlay.y * width / this.range, this.height, 
                        this.roundness, this.roundness, this.roundness, this.roundness );
        }
        for ( int i = 0 ; i < this.dragPositions.size() ; i++ ) {
            int position = this.dragPositions.get( i );
            ...
            text( this.min, this.x + this.min * width / range - ( this.height * 3 + 2 ) / 2, this.y + this.height - 10 );
            text( this.max, this.x + this.max * width / range - ( this.height * 3 + 2 ) / 2, this.y + this.height - 10 );
        }
    }
}